...
KEY_PROFILE = "xda"; // [riskeycap, gem, dsa, dcs, dss, kat, kam]
...
KEY_WIDTH = (KEY_UNIT*1.25-BETWEENSPACE); // The Y (NOTE: If using POLYGON_EGDES>4 this will be ignored)
...
KEY_ROTATION = [0,0.0,90]; // An example of how you'd rotate a keycap on its side.  Make sure to zoom in on the bottom to make sure it's *actually* going to print flat! This should be the correct rotation for riskeycap profile.  For GEM use:
...
DISH_TYPE = "domed"; // "inv_pyramid", "cylinder", "sphere" (aka "domed"), anything else: flat top
...
DISH_FN = $preview ? 28 : 128; // If you want to increase or decrease the resolution of the shapes used to make the dish (Tip: Don't go <64 for "cylinder" dish types and don't go <128 for "sphere")
...
STEM_SIDE_SUPPORTS = [1,1,0,0]; // Left, right, front, back
...
STEM_SUPPORT_DISTANCE = 0.5; // Controls the air gap between the stem and its support
